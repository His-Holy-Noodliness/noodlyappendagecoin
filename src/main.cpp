...
// Copyright (c) 2013-2014 The NoodlyAppendageCoin developers
...
uint256 hashGenesisBlock("0x45e34031cd491ea08d7224b91435c465cd40e0837642d6cfd1cccad93122e901");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Noodlyappendagecoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Noodlyappendagecoin Signed Message:\n";
...
{
    // Noodlyappendagecoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
	...
	return nSubsidy + nFees;
       ...
       // the FSM said "Let there be a Universe, or something!" And there was a Universe,
       // or something not terribly far off"
    ...
    // Subsidy is cut in half every 525600 blocks, which will occur approximately once per year
    nSubsidy >>= (nHeight / 525600); // Noodlyappendagecoin: 525600 blocks in ~1 year
...
static const int64 nTargetTimespan = 60 * 60; // Noodlyappendagecoin: Difficulty retargets every hour
static const int64 nTargetSpacing = 60; // Noodlyappendagecoin: 1 minute blocks
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    if (nActualTimespan < nTargetTimespan/16) // Noodlyappendagecoin: Maximum adjustment 16x or 1/16x
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Noodlyappendagecoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Noodlyappendagecoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xe10308da90e06801a2d7cdd940260ef27ee8830171de1d86bde57223de0a37ab");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1388632600;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1731642;

        if (fTestNet)
        {
            block.nTime    = 1388529549;
            block.nNonce   = 1425827;
        }
        ...
        assert(block.hashMerkleRoot == uint256("4fc0120077bbf01025690bb2b924f60729b43be8e63888c1be84d6efdeb48c74"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Noodlyappendagecoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    //// debug print
    printf("NoodlyappendagecoinMiner:\n");
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("NoodlyappendagecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("NoodlyappendagecoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static NoodlyappendagecoinMiner(CWallet *pwallet)
{
    printf("NoodlyappendagecoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("noodlyappendagecoin-miner");
    ...
    try { loop {
        ...
        printf("Running NoodlyappendagecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("NoodlyappendagecoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&NoodlyappendagecoinMiner, pwallet));
}
